...
const Header = () => {
  ...
  return (
    <>
      <header className="bg-white  w-screen  sticky top-0 shadow-lg"
        onMouseLeave={() => {
      ...
      >
        <nav className="flex justify-between items-center py-5 font-sans text-lg uppercase font-normal min-h-[90px]  px-10 sticky top-0 " id="nav">
          <a href="/" className="text-center">My Store</a>
              ...
              <li
                ...
                onMouseEnter={() => {
                  if (category?.subcategories?.length > 0) {
                    setCategories(categories.map((c) => c.name === category.name ? { ...c, isActive: true } : c))
                    setShowSubcategories({ show: true, categories: category.subcategories, name: category.name })
          ...
          <div className="flex items-center w-[400px] justify-end">
            <div className="min-w-[300px] max-w-[300px] rounded relative flex p-px items-center  border rounded" id="search-box" style={{
        ...
        <div id="sub-cat" className="ease-in h-[0]" style={{
          transition: "all .3s ease-in",
        }}>
          <ul className="flex justify-center space-x-5 bg-white uppercase">
            {showSubcategories.categories?.map((subcategory) => (
              <li key={subcategory}>
                <a href={`/${showSubcategories.name.toLowerCase()}/${subcategory.toLowerCase()}`} className="text-gray-600 hover:underline py-2">
                  {subcategory}
                </a>
              </li>
            ))}
        ...
        url.path !== "/" && (
          <div className="my-3 flex items-center text-sm px-10 py-5 ">
            {breadcrumbs.map((breadcrumb, index) => (
              <span key={breadcrumb}>
                <a href={breadcrumb === "" ? "/" : index > 1 ? `/${breadcrumbs[index - 1]}/${breadcrumb}` : `/${breadcrumb}`}
                  className="text-gray-400 hover:underline">
                  {breadcrumb === "" ? "Home" : breadcrumb.charAt(0).toUpperCase() + breadcrumb.slice(1)}
                </a>
